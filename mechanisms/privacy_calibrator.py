...
def subsample_epsdelta_inverse(eps, delta, prob):
    ...
    return np.log((np.exp(eps) - 1) / prob + 1), np.minimum(
        delta / prob, 1.0
    )


def subsample_epsdelta_get_prob(
    eps_target, delta_target, eps_base, delta_base
):
    """
    ...
    return np.mininum(
        ...
        np.minimum(
            ...
            (np.exp(eps_target) - 1) / (np.exp(eps_base) - 1),
        ),
    )
...
def RDP_mech(
    ...
    while np.sign(func(a)) == np.sign(func(b)):
        ...
        if count >= maxiter:
            # infeasible
            raise ValueError(
                "Infeasible privacy parameters for given RDP function and parameter bounds."
            )
...
def ana_gaussian_mech(epsilon, delta, tol=1.0e-12):
    ...
    if delta == delta_thr:
        ...
        if delta > delta_thr:
            ...
            predicate_left_BS = (
                lambda s: function_s_to_delta(s) > delta
            )
            function_s_to_alpha = lambda s: sqrt(
                1.0 + s / 2.0
            ) - sqrt(s / 2.0)
        ...
        predicate_stop_BS = (
            lambda s: abs(function_s_to_delta(s) - delta) <= tol
        )

        s_inf, s_sup = doubling_trick(predicate_stop_DT, 0.0, 1.0)
        s_final = binary_search(
            predicate_stop_BS, predicate_left_BS, s_inf, s_sup
        )
