...
class EkteloMatrix(LinearOperator):
    """
    An EkteloMatrix is a linear transformation that can compute matrix-vector
    products
    """

    def __init__(self, matrix):
        """Instantiate an EkteloMatrix from an explicitly represented backing
        matrix
...
class BlockDiag(EkteloMatrix):
    ...
    def matrix(self):
        return sparse.block_diag(
            [Q.matrix for Q in self.matrices], format="csr"
        )
...
class VStack(EkteloMatrix):
    ...
    def sparse_matrix(self):
        return sparse.vstack(
            [Q.sparse_matrix() for Q in self.matrices]
        )
...
class HStack(EkteloMatrix):
    ...
    def __mul__(self, other):
        if isinstance(other, VStack):
            # and shapes match...
            return Sum(
                [A @ B for A, B in zip(self.matrices, other.matrices)]
            )
        return EkteloMatrix.__mul__(self, other)
...
class Kronecker(EkteloMatrix):
    def __init__(self, matrices):
        self.matrices = matrices
        self.shape = tuple(
            np.prod([Q.shape for Q in matrices], axis=0)
        )
        self.dtype = np.result_type(*[Q.dtype for Q in matrices])
    ...
    def dense_matrix(self):
        return reduce(
            np.kron, [Q.dense_matrix() for Q in self.matrices]
        )

    def sparse_matrix(self):
        return reduce(
            sparse.kron, [Q.sparse_matrix() for Q in self.matrices]
        )
...
class Haar(EkteloMatrix):
    ...
    def matrix(self):
        ...
        for m in [2**c for c in range(self.k)]:
            ...
            B = sparse.kron(identity, [1, -1], format="csr")
