...
methods = ["advanced_extended+KWay", "localPGM+KWay"]
...
for epsilon in epsilons:
    ...
    inference_types = {"pgm_euclid":"PGM", "advanced_extended":"PrivPGD", "private_gsd": "PrivGSD", "gem":"GEM", "rap":"RAP ", "localPGM":"AP-PGM"}
    ...
    for plot_stat in statistics.keys():
        ...
        for (j,method) in enumerate(methods):
            ...
            for dataset in filtered_data.keys():
                ...
                if "localPGM" in method and plot_stat == "newl1_2Way_avg":
                    plot_stat_new = "l1_mean"
                elif "localPGM" in method and plot_stat =="wdist_1_2Way_avg":
                ...
                if  plot_stat == "synth_gradboost_test" and "regression" not in dataset:
                    ycompare[plot_stat][method][dataset] = 1- np.mean(filtered_data[dataset][name_run][plot_stat_new])
                else:
                    ycompare[plot_stat][method][dataset] = np.mean(filtered_data[dataset][name_run][plot_stat_new])
    ...
    with open(os.path.join(script_folder, f"counts_appgm_{epsilon}.tex"), 'w') as file:
